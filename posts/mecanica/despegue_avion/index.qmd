...
Un avión comercial es un laboratorio de física que además te lleva de vacaciones. Acelera de cero a más de 300 km/h en menos de un minuto, se despega del suelo, sube varios kilómetros y mantiene, mientras tanto, una burbuja de aire a presión distinta a la de afuera. Todo eso deja huellas medibles, y resulta que el instrumento para detectarlas ya viene en el bolsillo: un teléfono tiene acelerómetro, giroscopio, magnetómetro, barómetro y GPS.

La pregunta que quise responder es simple: **¿cuánto de la física de un despegue se puede reconstruir sin más equipo que un celular?**

La respuesta corta es: bastante más de lo que esperaba, incluyendo una cosa que no estaba buscando y que terminó siendo lo más interesante del experimento. El barómetro del teléfono no mide la atmósfera. Mide la cabina. Y esa diferencia, que al principio parecía un error de medición, es en realidad una medición directa —y accidental— del sistema de presurización del avión.

::: {.callout-note}
## Lo que vas a encontrar aquí
Tres cosas medidas, no estimadas: el instante exacto en que las ruedas dejan el piso, la diferencia entre despegar a 2 230 m y hacerlo a nivel del mar, y la brecha entre la altitud real del avión y la altitud que "siente" tu cuerpo dentro de la cabina.
:::
...
Todo el procesamiento está en Python (NumPy, SciPy, Matplotlib). Los archivos crudos exportados de phyphox y los scripts de análisis están en [ENLACE AL REPO].
...
Poner los dos vuelos en el mismo eje, con el origen en el despegue, hace visible lo que ninguna gráfica individual mostraba.

**La carrera de despegue en CDMX duró 48 s; la de Miami, 32 s.** Un 50 % más de tiempo sobre la pista para el mismo tipo de operación.

Esto tiene una explicación directa. La sustentación depende de la densidad del aire:

$$L = \tfrac{1}{2}\,\rho\, v^2 S\, C_L$$

A 2 230 m de altura la densidad del aire es aproximadamente un 20 % menor que a nivel del mar. Con menos $\rho$, el avión necesita más $v$ para generar la misma $L$, y llegar a esa velocidad mayor exige más pista y más tiempo. El aeropuerto de la Ciudad de México es conocido por esto entre pilotos: la altitud penaliza tanto la sustentación como el empuje de los motores.

### El hallazgo que no buscaba

Y aquí viene lo bueno. Miren el último panel de la primera figura.

Durante el ascenso desde CDMX, el GPS registra una subida real de **2 229 a 3 898 m**. La presión atmosférica exterior, a esas altitudes, debería caer de unos 780 a unos 630 hPa: **una caída de ~150 hPa**.

El barómetro del teléfono registró una caída de **8 hPa**.

No es un error del sensor. El barómetro está funcionando perfectamente — lo que pasa es que **no está midiendo la atmósfera, está midiendo la cabina**. El sistema de presurización del avión mantiene el interior a una presión mucho mayor que la exterior, y el teléfono, que va adentro, sólo puede medir lo de adentro.

Dicho de otro modo: mientras el avión subía 1 669 m reales, **la cabina "subió" unos 90 m**.

Hay un segundo detalle, más sutil, que aparece en los dos vuelos: **la presión de cabina sube ~6 hPa durante la carrera de despegue**, antes de que el avión se despegue del suelo. El sistema empieza a presurizar en tierra, de modo que la cabina arranca ligeramente por encima de la presión ambiente. En la gráfica de Miami esto se ve como una altitud de cabina que baja por debajo del nivel del mar justo antes del despegue.

## Discusión

### Qué mide realmente cada sensor

La lección metodológica del experimento es que **el sensor mide lo que lo rodea, no lo que uno quiere medir**. Suena obvio escrito así, pero es exactamente el error que casi cometo: aplicar la fórmula barométrica a los datos del teléfono y reportar "altitud del avión". El número que sale de ahí no es la altitud del avión; es la altitud equivalente de la cabina, que es una cantidad distinta y bastante más aburrida... salvo que uno se dé cuenta de que acaba de medir el sistema de presurización.

Esto explica también por qué el vuelo de regreso es más limitado: sin GPS no hay forma de conocer la altitud real, y la única señal disponible es precisamente la que no responde a la altitud real. Por eso en el reel de ese vuelo la trayectoria del avión es ilustrativa, no medida.

### Por qué la vibración es mejor detector que la aceleración

El resultado más sólido del análisis viene de tirar la información y quedarse con el ruido. La desviación estándar del acelerómetro no depende de la orientación del teléfono, ni de si el avión está acelerando, ni de la gravedad — sólo de si hay o no una fuente de vibración mecánica acoplada. Como el contacto rueda-pavimento es esa fuente, y desaparece de forma discreta, el detector es casi ideal.

Es un buen ejemplo de que **la parte "sucia" de una señal a veces contiene el dato más limpio**.
...
Fijar el teléfono a una superficie con orientación conocida, verificar que el GPS esté activo antes de despegar (el vuelo de regreso perdió esa mitad del experimento), y no dejar que la app se pause — los cortes ocultos son el problema más traicionero porque no dejan hueco visible en el eje de tiempo.
